keys = %Q[Qt::Key_Escape  0x01000000   
...
Qt::Key_Cancel  0x01020001   ].scan(/Qt\:\:Key\_([\w]+)/)
body = [];
alpha = "ABCDEFGHIJKLMNOPQRSTUVWXYZ"
alpha_down = alpha.downcase
keys.each do |k|
  key = k.first 
  if alpha.include? key then
body << %Q[\tif (type == "press" && keyToSend == "#{key}") {
\t\treturn new QKeyEvent(QEvent::KeyPress,Qt::Key_#{key},Qt::ShiftModifier);
\t}
\tif (type == "release" && keyToSend == "#{key}") {
\t\treturn new QKeyEvent(QEvent::KeyRelease,Qt::Key_#{key},Qt::ShiftModifier);
\t}
]
body << %Q[\tif (type == "press" && keyToSend == "#{key.downcase}") {
\t\treturn new QKeyEvent(QEvent::KeyPress,Qt::Key_#{key},Qt::NoModifier);
\t}
\tif (type == "release" && keyToSend == "#{key.downcase}") {
\t\treturn new QKeyEvent(QEvent::KeyRelease,Qt::Key_#{key},Qt::NoModifier);
\t}
]
  else
body << %Q[\tif (type == "press" && keyToSend == "#{key}") {
